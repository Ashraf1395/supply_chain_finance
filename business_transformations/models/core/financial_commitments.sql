...
WITH order_aggregates AS (
    ...
    FROM (select department_id, product_card_id,order_item_total,order_status from {{ ref('dim_order') }} limit 10000) o
...
department_market_category AS (
    ...
    JOIN (select department_id,department_name,market from {{ ref('dim_department') }} limit 10000) d ON oa.department_id = d.department_id
    JOIN (select product_card_id,category_name from {{ ref('dim_product') }} limit 10000) p ON oa.product_card_id = p.product_card_id
)
SELECT
    department_name,
    market,
